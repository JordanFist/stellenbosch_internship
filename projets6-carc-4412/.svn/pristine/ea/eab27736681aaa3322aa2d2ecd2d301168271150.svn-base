...
void test_init_next_nodes() {
  printf("Test de init_next_node .............");
  
  struct position pos = {0, 0};  
  struct tile *t = init_tile(0, NORTH, pos);
  
  assert(t->nodes[0]->next_nodes[0] == t->nodes[11]);
  assert(t->nodes[0]->next_nodes[1] == t->nodes[1]);

  struct tile *t2 = init_tile(23, NORTH, pos);
  assert(t2->nodes[11]->next_nodes[0] == t2->nodes[0]);
  assert(t2->nodes[1]->next_nodes[0] == NULL);
  ...
  for (int j = 0; j < 13; ++j) {
    for (int i = 0; i < 3; ++i) {
      printf("%d : %p\t", j, t2->nodes[j]->next_nodes[i]);
    }
    printf("\n");
  }
  */
